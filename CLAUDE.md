...
`line_send_hotspots` (`sql/line_hotspot_12_topn.sql` is the current definition — it has been rewritten four times since `line_hotspot_5_report.sql`, so always open the highest-numbered `line_hotspot_*` file, never this one) reports repeat-crash cells over a **sliding** window of `hotspotWindowDays` days counted back from `now()` — not the calendar month. Clustering is seeded-radius at `hotspotClusterMetres` (100 m): a centre is picked first and only points inside its circle join it, so a group can never chain outward along a road; intersections and U-turns match on straight-line distance, everything else on road distance. A cell is reported at `hotspotMinPerMonth` crashes **or** `hotspotSevereMinPerMonth` serious/fatal casualties, and one message per run lists up to `hotspotTopN` cells (5). Ranking is deaths-first, then a weighted score — death 12, serious-or-unconscious 6, crash 3, minor 3. Casualties are read out of `accidents.party1/party2` (plus their `passengers`), so legacy rows storing `"passengers":{}` must be type-checked with `jsonb_typeof` or the function dies. The dedupe `ref` embeds every count, which is what makes it re-send when a cell's numbers move and stay quiet when they don't — but under a sliding window the numbers move by themselves as old crashes age out, with nobody entering data. Paired with the original `*/5` cron, that re-sent the whole report roughly daily: 21 messages in 30 days, measured 8 Sep 2026. The `line-hotspot` cron is now `0 2 * * 1` (Monday 09:00 Bangkok — pg_cron reads UTC), set by `sql/line_hotspot_13_calmer.sql`. Cadence is the only knob that caps how **often** it sends; the thresholds and `hotspotTopN` only change how long and crowded each message is.
