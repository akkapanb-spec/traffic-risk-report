-- ตรวจว่าข้อความที่สั่งส่งไปแล้ว ไลน์รับจริงหรือไม่  ฉบับแก้ไข
-- ใช้ไฟล์นี้แทน line_verify_1.sql  ตัวเดิมติดตั้งแล้วใช้งานไม่ได้
--
-- ทำไมต้องมีฉบับแก้ไข
--   ตัวแก้ไขของ Supabase เติมแบ็กสแลชหน้าเครื่องหมายดอลลาร์ตอนลากไฟล์ลง
--   รูปแบบตรวจตัวเลขจึงกลายเป็น ตัวเลขที่ตามด้วยเครื่องหมายดอลลาร์ ซึ่งไม่ตรงกับอะไรเลย
--   ผลคือตัวตรวจจะไม่ยืนยันข้อความใดได้เลย โดยดูเหมือนทำงานปกติทุกอย่าง
--   ฉบับนี้จึงเลี่ยงเครื่องหมายดอลลาร์ ใช้วิธีตัดตัวเลขทิ้งแล้วดูว่าเหลืออะไรไหมแทน
--   เหลือว่าง แปลว่าเป็นตัวเลขล้วน  ได้ผลเท่ากันโดยไม่ต้องพึ่งอักขระที่ถูกแก้
-- ถ้ามีอักขระแปลกปลอมต่อท้ายบรรทัดสุดท้ายของไฟล์ ให้ลบออกก่อนกด Run
--
-- ปัญหาที่ไฟล์นี้แก้
...
begin
  ...
  with src as materialized (
    ...
    where detail like 'net_request_id=%'
      and translate(replace(detail, 'net_request_id=', ''), '0123456789', '') = ''
      and replace(detail, 'net_request_id=', '') <> ''
