...
begin
  ...
  -- รับเฉพาะหกคำที่พิมพ์อยู่บนอินโฟกราฟิก ไม่รับคำพ้องอื่น
  -- เหตุผลที่ไม่ใส่คำพ้องให้เยอะไว้ก่อน
  --   หนึ่ง  ในระบบของไลน์ยังมีการตอบกลับสำเร็จรูปตั้งค้างอยู่
  --          ถ้าคำพ้องของเราไปตรงกับคีย์เวิร์ดของเขา ประชาชนจะได้คำตอบสองฉบับพร้อมกัน
  --          แล้วตัวเลขในสองฉบับนั้นจะไม่ตรงกัน เพราะของเขาเป็นข้อความแช่ไว้
  --   สอง   สิ่งที่พิมพ์บนอินโฟคือสัญญาที่ให้ไว้กับประชาชน
  --          คำที่รับต้องเท่ากับคำที่โฆษณา ไม่มากไม่น้อย จะได้ตรวจสอบกันได้
  --   ถ้าวันหลังอยากรับคำพ้องเพิ่ม ต้องแก้อินโฟให้ตรงกันด้วย
  if v_q = 'สรุปวันนี้' then
    return line_msg_acc_day(null);

  elsif v_q = 'สรุปสัปดาห์' then
    return line_msg_acc_week(null);

  elsif v_q = 'สรุปเดือน' then
    return line_msg_pub_month();

  elsif v_q = 'สรุปปี' then
    return line_msg_pub_year();

  elsif v_q = 'ฝน' then
    return line_msg_pub_rain();

  elsif v_q = 'จุดเสี่ยง' then
    return array_to_string(array[
  ...
  -- คำนี้ไม่ได้พิมพ์บนอินโฟ เก็บไว้เป็นทางลัดสำหรับคนที่ลืมว่าพิมพ์อะไรได้
  -- ถ้าไม่ต้องการ ลบสองบรรทัดนี้กับข้อความข้างล่างออกได้
  elsif v_q = 'เมนู' then
    return array_to_string(array[
      ...
      '',
      'ต้องพิมพ์ให้ตรงตามนี้ ระบบจึงจะตอบ',
      '',
...
union all
select 'ลองถาม  จุดเสี่ยง', coalesce(line_pub_answer('จุดเสี่ยง'), 'ไม่ตอบ')
union all
select 'ลองถาม  สรุปสัปดาห์', coalesce(line_pub_answer('สรุปสัปดาห์'), 'ไม่ตอบ')
union all
select 'คำที่ต้องไม่ตอบ  สวัสดี', coalesce(line_pub_answer('สวัสดี'), 'ไม่ตอบ ซึ่งถูกต้อง')
union all
select 'คำที่ต้องไม่ตอบ  วันนี้', coalesce(line_pub_answer('วันนี้'), 'ไม่ตอบ ซึ่งถูกต้อง')
union all
select 'คำที่ต้องไม่ตอบ  สัปดาห์', coalesce(line_pub_answer('สัปดาห์'), 'ไม่ตอบ ซึ่งถูกต้อง');
