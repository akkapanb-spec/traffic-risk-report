-- เพิ่มประตูส่งข้อความอีกสองทาง  ฝนหนัก และ ฝนในเขตเทศบาล
-- ถ้ามีอักขระแปลกปลอมต่อท้ายบรรทัดสุดท้าย ให้ลบออกก่อนกด Run
-- เกณฑ์เดิม 15 เปอร์เซ็นต์ในวง 8 กม. ยังอยู่ครบทุกประการ ไฟล์นี้ไม่ได้เอาอะไรออก
-- จุดศูนย์กลางยังเป็นแยกเดชาติวงศ์เหมือนเดิม ไม่ได้ย้าย
--
-- ============================================================
...
select 'ประตู 2 ฝนหนัก' as รายการ,
...
union all
select 'ประตู 3 เขตเทศบาลวง 3 กม.',
  coalesce((select case when prosrc like '%rainYolkMinPct%' then 'ติดตั้งแล้ว' else 'ยังไม่มี' end
            from pg_proc p join pg_namespace n on n.oid=p.pronamespace
            where n.nspname='public' and p.proname='rain_alert_collect'),'ไม่พบ')
union all
select 'ตัวยิงคำถามยิงสองวง',
  coalesce((select case when prosrc like '%rainYolkUrl%' then 'ยิงแล้ว' else 'ยังยิงวงเดียว' end
            from pg_proc p join pg_namespace n on n.oid=p.pronamespace
            where n.nspname='public' and p.proname='rain_alert_fire'),'ไม่พบ')
union all
select 'ของเดิม ประตู 1 เกณฑ์วง 8 กม.',
...
select 'ของเดิม ช่วงเร่งด่วน',
  coalesce((select case when prosrc like '%rush:%' then 'อยู่ครบ' else 'หายไปแล้ว ต้องแก้' end
            ...
            where n.nspname='public' and p.proname='rain_alert_collect'),'ไม่พบ')
union all
select 'ของเดิม เพดาน 3 ครั้งต่อวัน',
...
select 'ของเดิม ตรวจภาพเรดาร์ค้าง',
  coalesce((select case when prosrc like '%rainSameCount%' then 'อยู่ครบ' else 'หายไปแล้ว ต้องแก้' end
            ...
            where n.nspname='public' and p.proname='rain_alert_collect'),'ไม่พบ')
union all
select 'ของเดิม เวลารอ 20 วินาที',
  coalesce((select case when prosrc like '%20000%' then 'อยู่ครบ' else 'หายไปแล้ว ต้องแก้' end
            from pg_proc p join pg_namespace n on n.oid=p.pronamespace
            where n.nspname='public' and p.proname='rain_alert_fire'),'ไม่พบ')
union all
select 'เกณฑ์ที่ใช้ตอนนี้',
  'วง 8 กม. ' || coalesce((select val #>> array[]::text[] from bs_settings where key='rainMinPct'),'15')
  || ' %   หรือ   วง 3 กม. ' || coalesce((select val #>> array[]::text[] from bs_settings where key='rainYolkMinPct'),'15')
  || ' %   หรือ   ฝนหนัก ' || coalesce((select val #>> array[]::text[] from bs_settings where key='rainHeavyMinPx'),'3') || ' จุด'
union all
...
union all
select 'ขั้นต่อไป', 'รอรอบหน้าอีก 7 นาที แล้วดูว่าช่อง yolk_pct ในตาราง rain_track มีตัวเลขเข้ามาไหม';
