-- ============================================================
-- ประกาศจุดควรหลีกเลี่ยง  งานหนึ่งวันประกาศทันที นอกนั้นรอบเก้าโมงเช้า
-- ============================================================
...
-- งานที่กินเวลาหลายวัน          ประกาศเก้าโมงเช้าของวันถัดจากวันที่กรอก
--
-- ย้ำว่านับจากวันที่กรอก ไม่ใช่วันที่งานเริ่ม
-- กรอกวันที่ 22 ก็ขึ้นเพจเช้าวันที่ 23 ต่อให้งานจะเริ่มเดือนหน้าก็ตาม
--
...
-- ส่วนงานหลายวันรู้ล่วงหน้าอยู่แล้ว ไม่ต้องรีบ รอเช้าวันรุ่งขึ้นได้
-- และได้ขึ้นเพจในเวลาที่คนอ่านกันจริง ไม่ใช่โผล่มาตอนไหนก็ไม่รู้
--
-- ------------------------------------------------------------
...
-- รอบที่ตรงกับเก้าโมงเช้าเท่านั้นที่จะหยิบงานหลายวันเพิ่มขึ้นมาด้วย
  ...
  for a in
    select t.id, t.images from traffic_advisories t
     where t.closed_at is null
       and t.ends_at   >= now()
       -- งานวันเดียว  ออกได้ทุกรอบ ขอแค่ยังไม่หมดเวลา
       -- งานหลายวัน   ออกได้เฉพาะรอบชั่วโมงเก้า และต้องกรอกมาก่อนวันนี้
       and (
         ( (t.starts_at at time zone 'Asia/Bangkok')::date
...
-- ==========================================================
-- 2  ขยับนาทีของตัวตั้งเวลาให้ตรงหัวชั่วโมง
-- ==========================================================
-- ของเดิมวิ่งนาทีที่ 7 22 37 52  รอบของชั่วโมงเก้าจึงตกที่ 09:07 ไม่ใช่ 09:00
-- ขยับเป็นนาทีที่ 0 15 30 45 เพื่อให้งานหลายวันขึ้นเพจตอนเก้าโมงตรงตามที่สั่งไว้
-- ความถี่เท่าเดิมทุกประการ เปลี่ยนแค่นาทีที่เริ่มนับ

select cron.schedule('fb-advisory', '0-59/15 * * * *', 'select fb_send_advisories(3)');

-- ==========================================================
-- 3  ตรวจ  ยังไม่มีอะไรถูกส่งออกไปจากไฟล์นี้
-- ==========================================================
...
select
  ...
  case
    ...
    when (t.created_at at time zone 'Asia/Bangkok')::date
       < (now() at time zone 'Asia/Bangkok')::date
      then 'ขึ้นเพจเก้าโมงเช้าวันนี้ ถ้ายังไม่ถึงก็พรุ่งนี้'
    else 'ขึ้นเพจเก้าโมงเช้าพรุ่งนี้'
