-- ============================================================
-- แก้ bs_site_profile เรียกไม่ได้ — ผมเขียนชื่อคอลัมน์ผิด
-- ============================================================
-- ต้องรัน bs_profile_1.sql มาก่อน ไฟล์นี้เขียนทับฟังก์ชันเดิมทั้งตัว
--
-- ตรวจก่อนกด Run: บรรทัดแรกของช่อง editor ต้องเป็นเส้น ==== ชุดนี้
--
-- อาการ
--   กดวิเคราะห์ในการ์ดโปรไฟล์จุดเสี่ยง แล้วขึ้นว่า
--   "ยังไม่ได้สร้างตารางหรือฟังก์ชันนี้" ทั้งที่รันไฟล์ไปแล้ว
--
-- สาเหตุ
--   บรรทัดที่ดึงชื่อถนนออกมา ผมเขียนว่า s.road_name
--   แต่ตาราง bs_sites ใช้ชื่อคอลัมน์ว่า road เฉย ๆ
--   ฐานข้อมูลจึงตอบว่า column ... does not exist
--   ซึ่งดันไปตรงกับข้อความเตือนตัวกลางของหน้าเว็บที่ดักคำว่า does not exist ไว้
--   หน้าเว็บเลยเดาว่ายังไม่ได้รันไฟล์ SQL ทั้งที่ฟังก์ชันมีอยู่จริง
--
--   คราวนี้ไม่ได้เดา — เทียบกับ create table ใน blackspot_1_tables.sql ตรง ๆ
--   คอลัมน์อื่นที่ฟังก์ชันนี้เรียกใช้ ตรวจครบแล้วว่ามีจริงทุกตัว
--   (title, latitude, longitude, radius_m, level, rule, acc_count,
--    fatal_count, published ของ bs_sites และ code, enabled ของ wx_zones)
-- ============================================================
...
begin
  ...
  select jsonb_build_object(
    ...
    'sites', coalesce((select jsonb_agg(jsonb_build_object(
        ...
        -- แก้ตรงนี้ — เดิมเขียน s.road_name ซึ่งไม่มีในตาราง
        'road', s.road,
...
-- ตรวจผลหลังรัน
-- ============================================================
-- กลับไปกดปุ่มวิเคราะห์ในการ์ดโปรไฟล์จุดเสี่ยงได้เลย
--
-- ถ้ายังขึ้นข้อความเดิมอีก แปลว่ามีที่ผิดอีกจุดที่ผมยังไม่เจอ
-- ให้รันบรรทัดล่างนี้ในหน้า SQL Editor จะเห็นข้อความ error เต็มพร้อมเลขบรรทัด
-- (token แอดมินหาได้จาก DevTools > Application > Local Storage
--  คีย์ accidentTrafficPoliceSession บนหน้าเจ้าหน้าที่ที่ล็อกอินอยู่)
--
--   select jsonb_pretty(bs_site_profile('<token>'));
--
-- ควรได้ sites เป็น array ของจุดเสี่ยงที่เผยแพร่แล้ว แต่ละจุดมี
-- n (จำนวนเหตุที่ตกในรัศมี) hours (24 ช่อง) weather และ spread_m
